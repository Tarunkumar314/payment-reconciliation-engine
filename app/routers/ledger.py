...
async def create_transaction(
    ...
    # ── 4. Atomic ledger write + outbox insert ───────────────────────────────
    # The OutboxEvent INSERT lives inside the SAME begin_nested() block as the
    # Transaction and LedgerEntry rows.  A single COMMIT covers all three.
    # This is the invariant that makes the outbox pattern correct:
    #   - If the DB write fails → all three rows roll back, no phantom event.
    #   - If COMMIT succeeds   → ledger and outbox are durable together;
    #     the relay can publish to Kafka at any future time.
    # Separating the outbox INSERT into a second commit would reintroduce the
    # same two-phase reliability gap the pattern exists to eliminate.
    event_type = (
        OutboxEventType.TRANSACTION_HELD
        if tx_status == TransactionStatus.HELD_FOR_REVIEW
